...
import com.backend.peluqueriaback.security.dto.JwtDto;
import com.backend.peluqueriaback.security.dto.LoginUsuario;
import com.backend.peluqueriaback.security.dto.NuevoUsuario;
import com.backend.peluqueriaback.security.entity.Usuario;
import com.backend.peluqueriaback.security.enums.RolNombre;
import com.backend.peluqueriaback.security.jwt.JwtProvider;
import com.backend.peluqueriaback.security.service.RolService;
import com.backend.peluqueriaback.security.service.UsuarioService;
...
public class AuthController {
	 ...
	 * 
	 * @PostMapping("/nuevo") public ResponseEntity<?> nuevo(@Valid @RequestBody
	 * NuevoUsuario nuevoUsuario, BindingResult bindingResult){
	 * if(bindingResult.hasErrors()) return new ResponseEntity(new
	 * Mensaje("campos mal puestos o email inválido"), HttpStatus.BAD_REQUEST);
	 * if(usuarioService.existsByNombreUsuario(nuevoUsuario.getNombreUsuario()))
	 * return new ResponseEntity(new Mensaje("ese nombre ya existe"),
	 * HttpStatus.BAD_REQUEST);
	 * if(usuarioService.existsByEmail(nuevoUsuario.getEmail())) return new
	 * ResponseEntity(new Mensaje("ese email ya existe"), HttpStatus.BAD_REQUEST);
	 * Usuario usuario = new Usuario(nuevoUsuario.getNombre(),
	 * nuevoUsuario.getNombreUsuario(), nuevoUsuario.getEmail(),
	 * passwordEncoder.encode(nuevoUsuario.getPassword())); Set<Rol> roles = new
	 * HashSet<>(); roles.add(rolService.getByRolNombre(RolNombre.ROLE_USER).get());
	 * if(nuevoUsuario.getRoles().contains("admin"))
	 * roles.add(rolService.getByRolNombre(RolNombre.ROLE_ADMIN).get());
	 * usuario.setRoles(roles); usuarioService.save(usuario); return new
	 * ResponseEntity(new Mensaje("usuario guardado"), HttpStatus.CREATED); }
	 * 
